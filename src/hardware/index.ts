...
export class SystemMonitor {
  ...
  async getSystemOverview(includeAnalysis: boolean = true): Promise<SystemOverview> {
  ...
  // 성능 병목지점 분석
  private analyzeSystemPerformance(overview: any) {
    ...
    // 프로세스 분석
    const highCpuProcesses = overview.processes.filter((p: any) => p.cpu > 20);
    if (highCpuProcesses.length > 0) {
      recommendations.push(`높은 CPU 사용률 프로세스: ${highCpuProcesses.map((p: any) => p.name).join(', ')}`);
    }
