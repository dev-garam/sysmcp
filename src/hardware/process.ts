...
export class ProcessMonitor {
  async getProcessList(sortBy: string = "cpu", limit: number = 10): Promise<ProcessInfo[]> {
    try {
  ...
  async getDetailedProcessStatus(): Promise<DetailedProcessStatus> {
    ...
    try {
      ...
      const result: DetailedProcessStatus = {
        summary,
        topProcesses,
        processTree,
        systemServices,
  ...
  private async executeDetailedPs(): Promise<string> {
    try {
      // 프로세스 상세 정보 수집
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('ps timeout')), 10000)
      );
  ...
  private async executeDetailedTop(): Promise<string> {
    try {
      // top을 사용한 상세 프로세스 정보
  ...
  private async executeLsofSummary(): Promise<string> {
    try {
      // 프로세스별 파일 디스크립터 요약
  ...
  private async executePstree(): Promise<string> {
    try {
  ...
  private parsePsSummary(psOutput: string) {
    ...
    try {
      if (!psOutput) return this.getEmptyPsSummary();
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (state.includes('R')) runningProcesses++;
        else if (state.includes('S') || state.includes('I')) sleepingProcesses++;
        else if (state.includes('Z')) zombieProcesses++;
  ...
  private parseDetailedProcesses(psOutput: string, topOutput: string, lsofOutput: string) {
    const fdCounts = this.parseLsofSummary(lsofOutput);

    const processMap = new Map();

    try {
      if (!psOutput) return this.getEmptyTopProcesses();
  ...
  private parseProcessTree(pstreeOutput: string, psOutput: string) {
    // 간소화된 프로세스 트리 분석
    try {
      const topParents: Array<{
        ...
        children: Array<{ pid: number; name: string; cpu: number; memory: number; }>;
      }> = [];
  ...
  private analyzeSystemServices(topCpuProcesses: any[]) {
    ...
    const heavyServices = topCpuProcesses
      ...
      .map(p => ({
        ...
        cpu: p.cpu,
        memory: p.memory,
        impact: (p.cpu > 20 || p.memory > 1000) ? 'high' as const :
          (p.cpu > 10 || p.memory > 500) ? 'medium' as const : 'low' as const
      }));
  ...
  private analyzeProcessSecurity(topCpuProcesses: any[]) {
    ...
    const suspiciousProcesses = topCpuProcesses
      .filter(p =>
        (p.cpu > 50 && !this.isKnownProcess(p.name)) ||
  ...
  private analyzePerformanceImpact(topCpuProcesses: any[], topMemoryProcesses: any[]) {
  ...
  // 유틸리티 메서드들
  private parseLsofSummary(lsofOutput: string): Map<number, number> {
    const fdCounts = new Map<number, number>();

    try {
      if (!lsofOutput) return fdCounts;

      const lines = lsofOutput.split('\n');
      for (const line of lines) {
        if (line.includes('PID FD_COUNT')) continue;
  ...
  private parseCpuTime(timeString: string): number {
    try {
  ...
  private isKnownProcess(name: string): boolean {
  ...
  private getSuspiciousReason(process: any): string {
    if (process.cpu > 50) return 'High CPU usage';
    if (process.memory > 2000) return 'High memory usage';
    if (process.resources.fileDescriptors > 1000) return 'Excessive file descriptors';
    return 'Unknown process with high resource usage';
  }

  private getCpuRecommendation(process: any): string {
    if (process.cpu > 80) return '프로세스 재시작 또는 종료 고려';
    if (process.cpu > 50) return '프로세스 최적화 또는 리소스 제한 설정';
    if (process.cpu > 30) return '백그라운드 작업 시간 조정';
    return '모니터링 지속';
  }

  private getMemoryRecommendation(process: any): string {
    if (process.memoryPercent > 20) return '메모리 누수 확인 및 프로세스 재시작';
    if (process.memoryPercent > 10) return '메모리 사용량 모니터링 강화';
    if (process.memoryPercent > 5) return '메모리 최적화 검토';
    return '정상 범위';
  }

  private createBasicProcessStatus(processes: ProcessInfo[]): DetailedProcessStatus {
