...
export class CpuMonitor {
  async getCpuStatus(): Promise<CpuStatus> {
  ...
  async getDetailedCpuStatus(): Promise<DetailedCpuStatus> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (process.platform === 'darwin') {
        ...
        const result: DetailedCpuStatus = {
          ...basicCpu,
  ...
  private async executeSysctlCpu(): Promise<string> {
    try {
      const commands = [
      ...
      const results = await Promise.all(
        commands.map(cmd => execAsync(cmd).catch(() => ({ stdout: '0' })))
      );
  ...
  private async executeTemperatureSensors(): Promise<string> {
    try {
  ...
  private async executeIostat(): Promise<string> {
    try {
      // 시스템 통계 정보
  ...
  private parseCpuDetails(sysctlOutput: string) {
  ...
  private parseTemperatures(temperatureOutput: string) {
    try {
      ...
      const sensors: { [key: string]: number } = {};
  ...
  private parseIostat(iostatOutput: string) {
