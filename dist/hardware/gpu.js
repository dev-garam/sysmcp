...
export class GpuMonitor {
    ...
    async getDetailedGpuStatus() {
        ...
        try {
            ...
            const [systemProfiler, ioregGpu, smctempGpu, powermetrics, activeProcesses] = await Promise.all([
    ...
    async executeSystemProfilerGpu() {
        try {
    ...
    async executeIoregGpu() {
        try {
            // GPU 관련 IORegistry 정보를 더 구체적으로 수집
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('IORegistry GPU timeout')), 3000));
            const commands = [
    ...
    parseGpuDetails(systemProfilerOutput, ioregOutput) {
        try {
    ...
    parseGpuTemperature(smctempOutput) {
        try {
    ...
    parseGpuProcesses(processOutput) {
        const processes = [];
        try {
            if (!processOutput)
                return processes;
