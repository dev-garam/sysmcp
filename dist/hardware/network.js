...
export class NetworkMonitor {
    ...
    async getDetailedNetworkStatus() {
        ...
        try {
            ...
            // 상세 네트워크 정보 수집 (병렬 실행)
            const [netstatOutput, nettopOutput, ifconfigOutput, wifiInfo, pingStats] = await Promise.all([
                this.executeNetstat(),
                this.executeNettop(),
    ...
    async executeNetstat() {
        try {
            // 활성 연결 정보와 통계
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('netstat timeout')), 5000));
            const commands = [
    ...
    async executeWifiInfo() {
        try {
            // WiFi 상세 정보 (macOS)
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('wifi info timeout')), 3000));
            const commands = [
    ...
    parseNetstat(netstatOutput) {
        try {
            ...
            if (stats) {
                ...
                if (tcpMatch)
                    protocolStats.tcp.connections = parseInt(tcpMatch[1]) || 0;
                if (udpMatch)
                    protocolStats.udp.connections = parseInt(udpMatch[1]) || 0;
            }
            return {
                activeConnections,
                establishedConnections,
    ...
    parseNettop(nettopOutput, basicStats) {
        try {
        ...
        try {
            ...
            for (let i = 1; i < interfaces.length; i += 2) {
                ...
                if (ifaceName.startsWith('en'))
                    ...
                    type = 'tunnel';
                else if (ifaceName.startsWith('awdl'))
                    type = 'airdrop';
